...
def test_get_episode(db):
    ep_in = EpisodeCreate(
        name="ep1", air_date="2022-03-04", segment=random_segment()
    )
...
def test_get_multi_episode(db):
    ep_in = EpisodeCreate(
        name="ep11", air_date="2022-03-04", segment=random_segment()
    )
    crud.episode.create(db, ep_in)
    ep_in = EpisodeCreate(
        name="ep12", air_date="2022-03-04", segment=random_segment()
    )
    crud.episode.create(db, ep_in)
